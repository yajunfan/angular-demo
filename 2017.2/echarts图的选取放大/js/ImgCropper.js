...
//容器对象,控制层,图片地址
    initialize: function(container, handle, url, options) {
        this._Container = $(container); //容器对象  ,echarts这个容器
        this._layHandle = $(handle); //控制层 选取区域的容器
        this.Url = url; //图片地址


        //this._layBase = this._Container;
         this._layCropper = this._Container
         this._layBase = this._Container.appendChild(document.createElement("img")); //底层   给这个容器中动态插入img标签
        //this._layCropper = this._Container.appendChild(document.createElement("img")); //切割层 给这个容器中动态插入img标签
       this._layCropper.onload = Bind(this, this.SetPos);  //这里绑定了一个方法
//用来设置大小
        this._tempImg = document.createElement("img");  //创建一个img标签
        this._tempImg.onload = Bind(this, this.SetSize);  //加载的时候给一个设置大小的方法

        this.SetOptions(options);  //设置默认属性的方法，参数是一个对象

        this.Opacity = Math.round(this.options.Opacity);//设置透明度
        this.Color = this.options.Color;  //颜色
        this.Scale = !!this.options.Scale;//放大缩小
...
//设置样式
        ...
        // this._layCropper.style.zIndex = 1;
