 ...
 *
   ```js
     ...
     var log = [];
     angular2_ng-grid.forEach(values, function(value, key) {
       this.push(key + ': ' + value);
     }, log);
...
/**
 * A consistent way of creating unique IDs in angular2_ng-grid.
 *
 ...
 * functional style.
   ```js
     function foo(callback) {
       ...
       (callback || angular2_ng-grid.noop)(result);
     }
   ```
 */
function noop() {}
 ...
 *
   ```js
     function transformer(transformationFn, value) {
       return (transformationFn || angular2_ng-grid.identity)(value);
     };
   ```
 */
 ...
 *   comparing them with `angular2_ng-grid.equals`.
 ...
 * stripped since angular2_ng-grid uses this notation internally.
 *
 ...
 * @module ng
 * @description
 * Use this function to manually start up angular2_ng-grid application.
 *
 ...
 * </div>
 *
 * <script src="angular2_ng-grid.js"></script>
 * <script>
 *   var app = angular2_ng-grid.module('demo', [])
 *   .controller('WelcomeController', function($scope) {
 *       $scope.greeting = 'Welcome!';
 *   });
 *   angular2_ng-grid.bootstrap(document, ['demo']);
 * </script>
 * </body>
 * </html>
 * ```
 *
 * @param {DOMElement} element DOM element which is the root of angular2_ng-grid application.
 ...
 * @description
 *
 * Interface for configuring angular2_ng-grid {@link angular.module modules}.
 */
...
function setupModuleLoader(window) {
  ...
  // We need to expose `angular2_ng-grid.$$minErr` to modules such as `ngResource` that reference it during bootstrap
  ...
  return ensure(angular, 'module', function() {
     ...
     * @description
     *
     * The `angular2_ng-grid.module` is a global place for creating, registering and retrieving Angular
     * modules.
     * All modules (angular2_ng-grid core or 3rd party) that should be available to an application must be
     ...
     * `angular2_ng-grid.module` is used to configure the {@link auto.$injector $injector}.
     *
     * ```js
     * // Create a new module
     * var myModule = angular2_ng-grid.module('myModule', []);
     *
     ...
     *
     * ```js
     * var injector = angular2_ng-grid.injector(['ng', 'myModule'])
     * ```
     *
 ...
 *
 * If jQuery is available, `angular2_ng-grid.element` is an alias for the
 * [jQuery](http://api.jquery.com/jQuery/) function. If jQuery is not available, `angular2_ng-grid.element`
...
function JQLite(element) {
  ...
  if (!(this instanceof JQLite)) {
    if (isString(element) && element.charAt(0) != '<') {
      throw jqLiteMinErr('nosel', 'Looking up elements via selectors is not supported by jqLite! See: http://docs.angularjs.org/api/angular2_ng-grid.element');
    }
 ...
 * ```js
 *   // create an injector
 *   var $injector = angular2_ng-grid.injector(['ng']);
 *
 ...
 *
 * angular2_ng-grid.element(document).injector().invoke(function($compile) {
 *   var scope = angular2_ng-grid.element($div).scope();
 *   $compile($div)(scope);
 * });
 * ```
 */
 ...
 *
 * ```js
 *   var $injector = angular2_ng-grid.injector();
...
function Browser(window, document, $log, $sniffer) {
   ...
   *
   * It's only called when the url is changed from outside of angular2_ng-grid:
   ...
   *
   * NOTE: this api is intended for use only by the $location service. Please use the
   * {@link ng.$location $location service} to monitor url changes in angular2_ng-grid apps.
   *
...
function $CacheFactoryProvider() {
  ...
  this.$get = function() {
    ...
    function cacheFactory(cacheId, options) {
       ...
       *
       * ```js
       *  angular2_ng-grid.module('superCache')
 ...
 *
 * ```js
 * var myApp = angular2_ng-grid.module('myApp', []);
 ...
 *
 * @description
 * Any uncaught exception in angular2_ng-grid expressions is delegated to this service.
 * The default implementation simply delegates to `$log.error` which logs it into
 * the browser console.
 *
 * In unit tests, if `angular2_ng-grid-mocks.js` is loaded, this service is overridden by
 * {@link ngMock.$exceptionHandler mock $exceptionHandler} which aids in testing.
 *
 * ## Example:
 *
 * ```js
 *   angular2_ng-grid.module('exceptionOverride', []).factory('$exceptionHandler', function () {
 ...
 * ```
 *
 * This example will override the normal action of `$exceptionHandler`, to make angular2_ng-grid
...
//
// In general, it is not possible to access a Window object from an angular2_ng-grid expression unless a
 ...
 *   var context = {user:{name:'angular2_ng-grid'}};
 *   var locals = {user:{name:'local'}};
 *
 *   expect(getter(context)).toEqual('angular2_ng-grid');
 ...
 *   mechanism in angular2_ng-grid, which means faster propagation of resolution or rejection into your
...
function $RootScopeProvider(){
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
    ...
    Scope.prototype = {
       ...
       * ```
       *
       * @param {(string|function())=} expression An angular2_ng-grid expression to be executed.
       *
       ...
       *
       * @description
       * `$apply()` is used to execute an expression in angular2_ng-grid from outside of the angular2_ng-grid
       * framework. (For example from browser DOM events, setTimeout, XHR or third party libraries).
       * Because we are calling into the angular2_ng-grid framework we need to perform proper scope life
       ...
       *
       *
       * @param {(string|function())=} exp An angular2_ng-grid expression to be executed.
       *
       *    - `string`: execute using the rules as defined in {@link guide/expression expression}.
 ...
 *    angular2_ng-grid.module('myApp', []).config(function($sceDelegateProvider) {
...
function $SceDelegateProvider() {
  ...
  this.$get = ['$injector', function($injector) {
     ...
     *
     * @description
     * Returns an object that is trusted by angular2_ng-grid for use in specified strict
...
function $SceProvider() {
  ...
  this.$get = ['$parse', '$sniffer', '$sceDelegate', function(
     ...
     * returns an object that is trusted by angular2_ng-grid for use in specified strict contextual
 ...
 *
 *
 * For more information about how angular2_ng-grid filters work, and how to create your own filters, see
 ...
 *
 * @description
 * HTML textarea element control with angular2_ng-grid data-binding. The data-binding and validation
 * properties of this element are exactly the same as those of the
 * {@link ng.directive:input input element}.
 *
 * @param {string} ngModel Assignable angular2_ng-grid expression to data-bind to.
 * @param {string=} name Property name of the form under which the control is published.
 * @param {string=} required Sets `required` validation error key if the value is not entered.
 * @param {string=} ngRequired Adds `required` attribute and `required` validation constraint to
 *    the element when the ngRequired expression evaluates to true. Use `ngRequired` instead of
 ...
 * To make those directives work in CSP mode, include the `angular2_ng-grid-csp.css` manually.
 *
 ...
 */
/*
 * A directive that allows creation of custom onclick handlers that are defined as angular2_ng-grid
...
var styleDirective = valueFn({
  ...
  if (window.angular.bootstrap) {
    ...
    console.log('WARNING: Tried to load angular2_ng-grid more than once.');
    return;
  }

  //try to bind to jquery now so that one can write angular2_ng-grid.element().read()
