...
define(['jquery'],function ($) {
    ...
    function drag(curEle) {
        ...
        $box.mousedown(function (e) {
            ...
            $(document).on('mousemove.move', function (e) {
                ...
                if (l <= -150) {
                    l = -150;
                } else if (l >= $(document).width() -  $(curEle).width()-160) {
                    l = $(document).width() -  $(curEle).width()-160;
    ...
    //选择房贷、信用贷、车贷
    $(document).on('mousedown',"select[name='myLink']",function () {
        var val=$('#myLink option:selected').val();
        console.log(val);
        if(val == "房贷"){
            $('.car').css('display','none');
            $('.houseAll').css('display','block');
        }else if(val == "车辆抵押"){
            $('.car').css('display','block');
            $('.houseAll').css('display','none');
        }
