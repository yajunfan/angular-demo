...
(function($){
    ...
    var Parseur = function(){
        var strip = function(s){ return s.replace(/^[\s\t]+|[\s\t]+$/g,'') };  //正则-- 用于匹配一到多个空格或者制表符等，并且将空格代替为无
        ...
        var lechs = function(s){
            ...
            var flush = function(){
                var bufstr = strip(buf)  //这表示将buf传入，将他里面的空格替换
                //如果有长度，就将这个传入数组中，否则就为空
...
//        			dragged.tempMass = 100000
    ...
    var IO = function(elt){
        var dom = $(elt);
        var db = $.couch.db('halfviz');//$.couch引用的是外部的js文化部
        ...
        var that = {
            ...
            saveable:function(isSaveable, savedUrl){
                ...
                if (!isSaveable && savedUrl){
                    ...
                    save.attr({target:"_blank", href:"tsconfig.json"})
    ...
    var HalfViz = function(elt){
        ...
        var _editing = false ;// whether to undim the Save menu and prevent navigating away是否undim保存菜单和防止导航
        var _failures = null;
        ...
        var that = {
            ...
            saveDoc:function(e){
                ...
                db.view('tsconfig.json', { key:graph_data.md5,
                    success:function(resp){
            ...
            //
            typing:function(e){ //如果按键为这几个的话，返回
                // trace(e.keyCode)
                var c = e.keyCode;  //获得键码
                if ($.inArray(c, [37, 38, 39, 40, 16])>=0){//$.inArray相当于原生的indexof，找到是0，找不到是-1；37-- +=键, 38 -- ，<键, 39 -- -_键, 40 -- .>键, 16 -- shift键
                    return
                }

                if (!_editing){ //_editing是false
                    that.io.saveable(true);
                    $.address.value("")
                }
                _editing = true;
