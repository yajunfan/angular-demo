...
//        			dragged.tempMass = 100000
  ...
  var IO = function(elt){
    var dom = $(elt)
    // var db = $.couch.db('halfviz');//使用arbor的方式获取到整个canvas以及控制的文本框这边的所有的元素
    ...
    var that = {
      ...
      exampleClick:function(e){
        var elt = $(e.target); //获取的是当前的li下的a元素
        var targetDoc = elt.closest('li').attr('class');//获取到当前a元素的父级元素的class名
        elt.closest('ul').find('a').removeClass('active');//获取到所有的a元素然后移除active这个class名，这是初始化
        elt.addClass('active');//给当前的a元素加上active的class名
      ...
      menuClick:function(e){
        var button = (e.target.tagName=='A') ? $(e.target) : $(e.target).closest('a') ;//closest() 方法获得匹配选择器的第一个祖先元素，从当前元素开始沿 DOM 树向上。总之要找的是examples对应的a标签
          console.log(button)  //--a.examples.selected
        var type = button.attr('class').replace(/\s?(selected|active)\s?/,'')
        console.log(type)  //结果是examples
        switch(type){
        case "examples":
          var toggled = button.hasClass('selected');//判断是否有selected这个class名，有就隐藏下面的下拉列表，没有就显示
        ...
        case "new":
          that.hideExamples(); //是new的话就直接隐藏这个下拉列表
          $(that).trigger({type:"clear"});//这里就是激活文本的事件
          break
  ...
  var HalfViz = function(elt){
    ...
    var that = {
      ...
      updateGraph:function(e){ //这里就是直接进行网页更改的时候进行的操作实时更新
