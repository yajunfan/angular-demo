...
Dates.getPath = (function(){
    ...
    return config.path ? config.path : jsPath.substring(0, jsPath.lastIndexOf("/js") + 1);
}());
...
Dates.orien = function(obj, pos){
    ...
    if(rect.bottom + obj.offsetHeight/1 <= Dates.winarea()){
        tops = rect.bottom - 1;         
    } else {
        tops = rect.top > obj.offsetHeight/1 ? rect.top - obj.offsetHeight + 1 : Dates.winarea() - obj.offsetHeight;
    }
